...
All weights are configurable via :class:`RiskProfile` so teams can tune the
scoring to their own codebase characteristics without forking.
"""
...
@dataclass
class RiskProfile:
    """Tunable weights for :func:`risk_score` and :func:`combined_risk`.

    Defaults match the original hard-coded values (ARCHITECTURE 4.3).
    """

    complexity: float = 0.5
    goto_density: float = 0.2
    external_calls: float = 0.15
    undocumented_literals: float = 0.1
    cascade_multiplier: float = 0.5
    trigger_chain_multiplier: float = 0.8
...
def risk_score(node: Node, profile: RiskProfile | None = None) -> float:
    if profile is None:
        profile = RiskProfile()
    p = node.properties
    ...
    raw = (
        ...
        + goto * profile.goto_density
        + ext_calls * profile.external_calls
        + literals * profile.undocumented_literals
    )
...
def combined_risk(store: GraphStore, node: Node, base: float, profile: RiskProfile | None = None) -> float:
    if profile is None:
        profile = RiskProfile()
    score = base
    if node.type == NodeType.DB2_TABLE:
        score += cascade_depth(store, node.id) * profile.cascade_multiplier
    elif node.type == NodeType.TRIGGER:
        score += trigger_chain_depth(store, node.id) * profile.trigger_chain_multiplier
    return score