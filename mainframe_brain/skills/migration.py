...
class MigrationScout:
    ...
    def analyze(self, store: GraphStore) -> list[SkillOutput]:
        ...
        lines = [
            ...
            "description: Top modernization candidates ranked by risk and complexity",
            "category: migration",
            "---",
            "",
            f"# Migration Candidates — Top {len(top)} Programs",
            "",
            "Ranked by risk score (complexity + GOTO density + external calls + cascade/trigger depth).",
            "Higher scores = harder to maintain, higher ROI for modernization.",
            "",
            "| Rank | Program | Risk Score | Complexity | GOTO Density | Ext Calls |",
            "|------|---------|------------|------------|--------------|-----------|",
        ]
        ...
        for i, (prog, risk) in enumerate(top, 1):
            ...
            if p.get("cyclomatic_complexity", 0) > 10:
                lines.append(
                    f"- **High complexity** ({p['cyclomatic_complexity']}): "
                    "consider extracting sub-procedures"
                )
            if p.get("goto_density", 0) > 0.3:
                lines.append(
                    f"- **GOTO-heavy** ({p['goto_density']:.1%} density): restructure control flow first"
                )
            calls = len(p.get("external_calls", []) or [])
            if calls > 5:
                lines.append(f"- **{calls} external calls**: mock these for testing")
            lines.append(
                f'- View details: `mainframe-brain query --store-path brain.db "impact of {prog.name}"`'
            )
            lines.append("")
